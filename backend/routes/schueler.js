...
router.post('/', auth, adminOnly, async (req, res) => {
  ...
  try {
    const result = await pool.query(
      ...
      [vorname,nachname,geburtsdatum,schule,klasse,faecher,eltern_name,eltern_tel,eltern_email,adresse,but_status,but_zeitraum_von,but_zeitraum_bis,diagnose,notizen]
    );
...
router.put('/:id', auth, adminOnly, async (req, res) => {
  ...
  try {
    const result = await pool.query(
      ...
      [vorname,nachname,geburtsdatum,schule,klasse,faecher,eltern_name,eltern_tel,eltern_email,adresse,but_status,but_zeitraum_von,but_zeitraum_bis,diagnose,notizen,req.params.id]
    );
    res.json(result.rows[0]);
  } catch (err) {
    res.status(500).json({ error: err.message });
...
// Zuweisungen abrufen
router.get('/:id/zuweisungen', auth, adminOnly, async (req, res) => {
  try {
    const result = await pool.query(
...
router.delete('/:id/zuweisung/:lehrkraft_id', auth, adminOnly, async (req, res) => {
  try {
    await pool.query(
      `DELETE FROM lehrkraft_schueler WHERE schueler_id=$1 AND lehrkraft_id=$2`,
      [req.params.id, req.params.lehrkraft_id]
    );
    res.json({ success: true });
  } catch (err) {
    res.status(500).json({ error: err.message });
