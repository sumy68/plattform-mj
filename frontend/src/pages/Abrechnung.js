...
export default function Abrechnung() {
  ...
  const loadAuszahlungen = async () => {
    try {
      const res = await axios.get(`${API}/api/abrechnung/auszahlungen?monat=${monat === 'alle' ? '' : monat}`, { headers: { Authorization: `Bearer ${localStorage.getItem('token')}` } });
  ...
  const loadAdminStats = async () => {
    ...
    allStunden.forEach(st => {
      if (!byLehrkraft[st.lehrkraft_id]) {
        ...
        byLehrkraft[st.lehrkraft_id] = { id: st.lehrkraft_id, name: st.lehrkraft_name, role: lk?.role || 'lehrkraft', stundensatz: parseFloat(st.lehrkraft_stundensatz || lk?.stundensatz || 0), stundensatz_2er: parseFloat(st.lehrkraft_stundensatz_2er || lk?.stundensatz_2er || 0), stundensatz_3er: parseFloat(st.lehrkraft_stundensatz_3er || lk?.stundensatz_3er || 0), absage_stundensatz: parseFloat(st.lehrkraft_absage_stundensatz || lk?.absage_stundensatz || 0), stunden: [] };
      }
    ...
    const lehrkraefte = Object.values(byLehrkraft).map(lk => ({
      ...
      betrag_gesamt: lk.stunden.reduce((sum, s) => { const satz = s.kurzfristige_absage ? (lk.absage_stundensatz || lk.stundensatz) : (s.unterrichtsform === '2er' ? (lk.stundensatz_2er || lk.stundensatz) : s.unterrichtsform === '3er' ? (lk.stundensatz_3er || lk.stundensatz) : lk.stundensatz); return sum + (parseFloat(s.dauer_minuten)||0)/60*satz + (s.fahrt_km ? parseFloat(s.fahrt_km)*0.38 : 0); }, 0),
      betrag_offen: lk.stunden.filter(s => !s.abgerechnet).reduce((sum, s) => { const satz = s.kurzfristige_absage ? (lk.absage_stundensatz || lk.stundensatz) : (s.unterrichtsform === '2er' ? (lk.stundensatz_2er || lk.stundensatz) : s.unterrichtsform === '3er' ? (lk.stundensatz_3er || lk.stundensatz) : lk.stundensatz); return sum + (parseFloat(s.dauer_minuten)||0)/60*satz + (s.fahrt_km ? parseFloat(s.fahrt_km)*0.38 : 0); }, 0),
    }));
