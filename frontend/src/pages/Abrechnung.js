  ...
  const loadGuthaben = async () => {
    ...
    try {
      ...
      if (user?.role === 'lehrkraft') {
        ...
        setGuthaben({ stunden: offenRes.data.stunden, stundensatz: offenRes.data.stundensatz, bereits_abgerechnet: 0, gesamt_betrag: offenRes.data.gesamt_betrag });
      } else {
    ...
    <div>
      ...
      {adminStats && <>
        <div className="stats-grid" style={{marginBottom:24}}>
          ...
          <div className="stat-card"><div className="stat-number" style={{color:'var(--danger)'}}>{auszahlungen.reduce((sum,a)=>sum+parseFloat(a.betrag),0).toFixed(0)} €</div><div className="stat-label">Gesamtkosten</div></div>
