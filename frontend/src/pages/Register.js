...
const emptyForm = { name: '', email: '', password: '', password2: '', role: 'honorarkraft' };
...
export default function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      await axios.post(`${API}/api/auth/register-request`, { name: form.name, email: form.email, password: form.password, role: form.role });
      setSuccess(true);
    } catch (err) {
  ...
  if (success) return (
    <div className="login-page">
      <div className="login-card" style={{ textAlign: 'center' }}>
        ...
        <Link to="/login" style={{ color: 'var(--purple)', fontWeight: 700, textDecoration: 'none' }}>→ Zurück zum Login</Link>
      </div>
    </div>
  );
      ...
      <div className="login-card" style={{ maxWidth: 480 }}>
        ...
        <form onSubmit={handleSubmit}>
          <div className="form-group">
            ...
            <input type="text" required value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} placeholder="Vor- und Nachname"/>
          </div>
          <div className="form-group">
            <label>E-Mail *</label>
          ...
          <div className="form-group">
            ...
            <div style={{ display: 'flex', gap: 12 }}>
              {[
                ...
                <div key={opt.value} onClick={() => setForm({ ...form, role: opt.value })} style={{ flex: 1, padding: '12px 16px', borderRadius: 10, cursor: 'pointer', border: `2px solid ${form.role === opt.value ? 'var(--purple)' : 'var(--lavender)'}`, background: form.role === opt.value ? 'var(--purple-pale)' : 'white', textAlign: 'center', transition: 'all 0.2s' }}>
        ...
        <p style={{ textAlign: 'center', marginTop: 20, fontSize: 14, color: 'var(--text-light)' }}>
          Bereits registriert?{' '}
          <Link to="/login" style={{ color: 'var(--purple)', fontWeight: 700, textDecoration: 'none' }}>Anmelden</Link>
        </p>
      </div>
    </div>
  );
